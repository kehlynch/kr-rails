...
const Bids = ({
  ...
  makeBid,
  nextPlayerName,
}: BidsProps): React.ReactElement => {
  const finishedText =
    declarableType === 'bid' ? 'bidding finished' : 'announcements finished'
  const actionText =
    declarableType === 'bid' ? 'make a bid' : 'make an announcement'
  return (
    <div className="flex">
      ...
      {bidsByPosition(bids).map((slugs, position) => {
        ...
        return (
          <BidIndicators
            slugs={slugs}
            key={`bids-${position}-${slugs.join(',')}`}
            compassName={compassName(position, myPosition)}
          />
        )
